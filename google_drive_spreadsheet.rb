...
class InputSheet
	...
	def update(num, sold, sell)
		...
		@sheet[row, @rows.index(:ebay_sold_url) + 1] = sold[:url];
                # @sheet[row, @rows.index(:ebay_sold_price) + 1] = @exchange.to_JPY(sold[:market_price]) if sold[:market_price]
		...
		# @sheet[row, @rows.index(:ebay_sell_price) + 1] = @exchange.to_JPY(sell[:market_price]) if sell[:market_price]
...
class OutputSheet
	...
	def update(product)
		row = check(product);
		if(row != nil)
			puts "Update yahoo item: " + p.title + " at " + row.to_s;
		...
		#	@sheet[row, 13] = product.data[:purchase_price];	# 入札上限 
	...
	def update_ebay(row, data)
		#@sheet[row, 13] = data[:purchase_price];
